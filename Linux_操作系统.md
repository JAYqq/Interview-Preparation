...
系统调用涉及到用户态和内核态的过渡，可以参考上面的CPU上下文切换。
...
顺序读指的是IO操作的磁盘扇区前后两次是相隔很近的，随机读则相反。所以随机读会使得无端多了很多磁盘的寻道时间。

### 阻塞和非阻塞

https://zhuanlan.zhihu.com/p/22707398

![](https://pic3.zhimg.com/80/v2-6507ab3517814b1b84fbff9a3eb31842_1440w.png)

比如IO操作，在发起读取文件的请求时，应用层会调用系统内核的I/O接口。
...
而对于**异步型的调用**，应用层无需主动向系统内核问询，在系统内核读取完文件数据之后，会主动通知应用层数据已经读取完毕，此时应用层即可以接收系统内核返回过来的数据，再做其他事情。
